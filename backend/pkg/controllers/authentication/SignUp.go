...
func HandleSignup(response http.ResponseWriter, request *http.Request) {
	...
	insertErr := database.HandleDatabaseInsert("users", user.Email, user.Phone, user.Password, user.First_name, user.Last_name, user.User_id, user.Created_at, user.Updated_at, user.Token, code, agent)
